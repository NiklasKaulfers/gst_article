...
\subsection{Integration Tests - Erstellung und Abspeicherung eines Speichercodes}
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}[node distance=3cm]
            \node (start) [startstop] {Spiel starten};
            \node (user1) [decision, below of=start] {Spieleraktion};
            \node (pressSave) [process, right of=user1, xshift=2cm] {Speicherknopf};
            \node (play) [process, below of=user1] {Aktionen auf dem Spielfeld};
            \node (loadGame) [process, left of=user1, xshift=-2cm] {Ladenknopf};
            \node (createSavecode) [process, below of=pressSave] {erstellen Speicherdatei};
            \node (accessSavecode) [process, below of=loadGame] {laden der Speicherdatei};
            \draw [arrow] (start) -- node {} (user1);
            \draw [arrow] (user1) -- (pressSave);
            \draw [arrow] (createSavecode) -- (user1);
            \draw [arrow] (pressSave) -- (createSavecode);
            \draw [arrow] (user1) -- (play);
            \draw [arrow] (play) -- (user1);
            \draw [arrow] (user1) -- (loadGame);
            \draw [arrow] (loadGame) -- (accessSavecode);
            \draw [arrow] (accessSavecode) -- (user1);
        \end{tikzpicture}
        \caption{Ablauf des Integration Tests}\label{gph:IntegrationTest}
    \end{figure}
