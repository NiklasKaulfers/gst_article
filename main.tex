...
\subsection{Arten der dynamischen Tests}
    ...
    Im Umfeld von Objektorientierter Programmierung (OOP) heißt dies vor allem Testen der einzelnen Klassen, da diese als einziges in Isolation getestet werden können\footfullcite[vgl. ][S. 1925f.]{wappler2006evolutionary}.
    Da die Umsetzung von OOP etwas mangelhaft in diesem Projekt ist und keine tiefgreifende Modualisierung stattfindet,
    ...
    Diese Tests werden an Code durchgeführt, welcher sich verändert hat\footfullcite[vgl. ][S. 583f.]{legunsen2016extensive}.\\
    ...
    In diesem Beispiel wird genannt, dass Fehler in einem Modul immer dazu führen, dass die davon abhängigen Module davon beeinträchtigt werden \footfullcite[S. 134f.]{zhenyi1998coupling}.
...
\subsection{Arten der statischen Tests}
    ...
    Diese pullbasierten Reviews beziehen sich auf Reviews von Pullrequests, beispielsweise über Git\footfullcite[vgl. ][S. 182]{sadowski2018modern} oder genauer GitHub.\\
    Eine weitere Form ist die \textbf{statische Analyse}, diese bezieht sich auf das Betrachten einer Komponente im Bezug auf Form, Struktur, Inhalt und Dokumentation\footfullcite[S. 70]{ieee1990stdGlossary}.
...
\section{Einfluss von KI/LLMs}
    Large Language Models (LLM) sind in den letzten Jahren zu einem teils durchaus nützlichen Tool in der Softwareentwicklung geworden.
    Jedoch sind diese Tools vor allem im Bereich des Softwaretestens heute von hoher Bedeutung und von großem Nutzen.
    ...
    Während diese Arbeit von vielen Ungenauigkeiten von LLMs redet\footfullcite[S. 917]{wang2024software},
    sehen andere ein größeres Risiko im menschlichen Error und LLMs und KI im generellem als möglichen Weg diese zu reduzieren\footfullcite[S. 5f.]{baqar2025future}.
    Wieder andere Artikel sind weniger optimistisch und sehen LLMs als sinnvolles Tool, welches jedoch Limits hat.
    Beispielsweise das Wort Bank im Zusammenhang mit Geldinstitut oder Sitzgelegenheit und inwiefern eine LLM hier in der Lage ist den richtigen Kontext zu ermitteln\footfullcite[S. 21f.]{alenezi2025ai}.
    Generell können LLMs jedoch von großem Nutzen vor allem, wen es um das Schreiben von Tests, welche sehr einfach zu verstehen sind und für den Mensch nur repetetiv sein können.
    Beispielsweise bekannte Algorithmen in Unittests, jedoch ist auch hier menschliches Überprüfen noch von nöten.\\
    Zudem ist es hier sinnvoll anzumerken, dass KI/LLMs im Zusammenhang mit Softwareentwicklung und Softwaretestens eine relativ neue Technologie ist und selbst die Arbeiten von 2024 und 2025 nicht mehr zwingend aktuell sind.
...
\subsection{System Testing}
    ...
    Vor allem in sehr großen Projekten ist diese häufig zu komplex\footfullcite[vgl. ][S. 199]{hayes1999}.
...
\section{statische Tests}
    ...
    Hier wird der Commit \texttt{dec55bf} betrachtet, welcher die Funktion zum Speichern und Laden des Speicherstands hinzufügte.
...
\section{KI-basierte Tests}
    ...
    Um die KI zu testen wurden im Rahmen dieser Arbeit mehrere Modelle zur Testgenerierung verwendet.
    Es wurde stets der Prompt\ \ref{pro:testingPrompt}.
...
\subsection{GitHub Copilot mit GPT 5o-mini}\label{sec:gpt5o}
    GitHub Copilot ist eine in das Integrated Development Environment (IDE) integrierter KI Assistent, 
    ...
    GPT 5o-mini war hier in der Lage, sowohl die Datei ordentlich zu lesen und zu verstehen,
    als auch entsprechende Ausführung des Skripts durchzuführen.
    Hier versuchte es zuerst gradle, da das Projekt in der Ausführung jedoch kein gradle hat,
    hat die KI dann entsprechende Terminalanweisungen zur Ausführung dieser Tests mit javac und jar erstellt und selbstständig ausgeführt.
    ...
    Anzumerken ist hier auch, dass Copilot Zugriff auch auf andere Dateien hat, so sind die Testnamen ähnlich der bereits bestehenden Tests.
...
\subsection{Claude Sonnet 4.5 im Browser}
    ...
    Demnach sollte es auch gut für eine Entwicklung von Tests sein.
    ...
    Auch hierfür musste zuerst der Prompt\ \ref{pro:promptAfterTest} verwendet werden, da Claude nicht selbstständig so etwas erledigt.
    ...
    Der Erfolg hier ist beschränkter, zwar ist der Code richtig und befasst sich mit den richtigen Ansätzen, 
    jedoch ist hier noch eine Menge eigenes Einwirken von Nöten. 
\subsection{Vergleich}
    Die grundlegenden Tests sind bei beiden Fällen nahezu gleich und decken die selben Äquvalenzklassen wie in der Grafik\ \ref{gph:validInputIsTopOfColumn} ab.
    Auch werden in beiden Fällen Steine platziert um das erwartete Verhalten bei steigender Steinezahl pro Spalte zu testen.
    Der Zugriff auf die gesamte Codebasis, welchen GitHub Copilot hat ist zudem sehr stark in den Resultaten zu spüren.
    ...
    und kontextbasierte Anweisungen zu erstellen, welche die Testsuit selbstständig ausgeführt haben.
    Im Gegensatz dazu hat Claude Sonnet im Browser sehr schwach den Kontext verwendet und viel Architektur des Projektes angenommen, ohne das diese tatsächlich stimmte.\\
    ...
    Die hier angegebenen Testfälle sind verhältnismäßig sehr einfach zu lösen.
...
\section{Nicht behobene Probleme}
    ...
    So besteht weiterhin eine schlechte Grundstruktur mit wenigen und zu großen Dateien, welche zudem zu viele Tätigkeiten gleichzeitig machen.
    Auch wurden nicht alle Errors behoben, so besteht der Error, bei welchem der Nutzer einen Speicherstand laden kann, es jedoch keine Speicherstand gibt und das Programm somit abstürzt.
    Zudem sind grundsätzliche Logikfehler im Code nicht behoben, da dies nicht im Rahmen der Arbeit ist und das Projekt keine weitere Arbeit erfährt.
\section{Behobene Probleme}
    Obwohl noch viele Probleme existieren konnten trotzdem einige behoben werden.
    Dies vor allem im Hinblick auf Testdeckung. 
    Die bereits stehenden Tests zu vor dem Beginn dieser Arbeit waren bei weitem unzureichend,
    das Problem bestand vor allem darin, das es keine spezifischen Tests der einzelnen Bestandteile gab, sondern nur generelle Tests, in welchen mehrere Features auf einmal getestet wurden.
...
\chapter{Fazit}
    ...
    Ein Projekt wie dieses sollte nicht nochmal entstehe, da es nahezu nicht wartbar ist und nicht ausreichend Modular ist.
    ...
    Generell sind im Verlauf der Arbeit häufig Fehler aufgetreten, welche vor allem der Unerfahrenheit zu verschulden waren.\\
    Auch zeigt sich, dass vor allem für einfache Tests KI gewaltige Zeit- und Ressourceneinsparungen erzeugt.
    Solche Tests, wie hier in Abschnitt\ \ref{sec:unitTests}, werden in Zukunft wahrscheinlich nicht mehr menschlich geschrieben sein.
\begin{appendices}
\chapter{Grafiken}
    \begin{figure}
